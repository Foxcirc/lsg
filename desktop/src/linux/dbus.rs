...
use std::{borrow::Cow, collections::HashMap, convert::identity, env, error::Error as StdError, fmt, future, hash::{Hash, Hasher}, io::{self, Read, Write}, iter, mem, os::{fd::{AsRawFd, OwnedFd}, unix::net::UnixStream}, time::Duration};

use futures_lite::FutureExt;

use crate::{Event, EvlError, NotifEvent};
...
async fn initial_auth(stream: &mut async_io::Async<UnixStream>) -> io::Result<()> {
    ...
    let mut buf = [0; 128];
    let len = unsafe { stream.read_with_mut(|it| it.read(&mut buf)) }.await?;
    if &buf[..2] != b"OK" {
        let msg = format!("sasl authentication failed, got following reply: {:?}", &buf[..len]);
    ...
    let data = msg.serialize(Serial::NOREPLY); // don't care about the serial
...
fn process_incoming(con: &mut Connection) -> Result<Incoming, ParseError> {

    let result = GenericMessage::deserialize(&con.buf);
    match result {

        Ok((offset, mut msg)) => {

            // remove the data that was parsed
            con.buf.drain(..offset);
...
async fn process_request(con: &mut Connection, request: Outgoing) -> io::Result<()> {

    match request {

        Outgoing::InitialAuth => {
            // lazily perform the auth
            initial_auth(&mut con.bus).await?;
        }

        Outgoing::MethodCall(serial, payload) => {

            println!("doing call: serial = {}, member = {:?}", serial, payload.member);

            let msg = payload.serialize();
            let data = msg.serialize(Serial { inner: serial });

            let stream = unsafe { con.bus.get_mut() };
            stream.write_all(&data)?;

        },

        Outgoing::MethodCallVoid(mut payload) => {

            println!("doing call (void): member = {:?}", payload.member);

            payload.no_reply = true;

            let msg = payload.serialize();
            let data = msg.serialize(Serial::NOREPLY);
        ...
        Outgoing::CloseNotif(serial) => {
            con.close_notif(serial);
        }
...
pub struct Connection {
    ...
    sender: async_channel::Sender<Outgoing>,
    receiver: async_channel::Receiver<Outgoing>,

    notifs: HashMap<NotifId, OutgoingId>,

    /// the bus connection
    bus: async_io::Async<UnixStream>,
    /// current serial, counting up from 1
    serial: u32,
    /// current incoming steam buffer
    buf: Vec<u8>,
...
impl Connection {
    ...
    pub fn new<S: Into<String>>(app: S) -> io::Result<Self> {
        ...
        sender.try_send(Outgoing::InitialAuth).unwrap();
        ...
        // setup for managing notifications
    ...
    pub async fn next<T>(&mut self) -> Result<Event<T>, EvlError> {
        ...
        loop {
            ...
            if let Incoming::Reply(serial, result) = incoming {
                ...
                if let Some(it) = self.notifs.get_mut(&serial) {
                ...
                if signal.iface == "org.freedesktop.Notifications" {
                    ...
                    if signal.member == "ActionInvoked" {
                        ...
                        id = signal.get(0)?;
                        let name = signal.get(1)?;

                        event = NotifEvent::ActionInvoked {
                            action: InvokedNotifAction { name },
                        };

                    } else if signal.member == "NotificationClosed" {
                        id = signal.get(0)?;
                        event = NotifEvent::ActionInvoked {
                            action: InvokedNotifAction { name: "closed".into() },
                        };

                    } else {
                        panic!("invalid dbus notifcation event"); // TODO: error not panic
                    }
    ...
    async fn flush(&mut self) -> Result<(), EvlError> {
        ...
        while let Ok(req) = self.receiver.try_recv() {
            ...
            if self.receiver.len() == 0 &&
                matches!(req,Outgoing::InitialAuth) { return Ok(()) };

            process_request(self, req).await?;
    ...
    pub fn send_notif(&mut self, notif: &NotifBuilder<'_>) -> Notif {
        ...
        call.arg(self.app.clone()); // app name
        ...
        let actions: Vec<String> = notif.actions.iter()
    ...
    fn close_notif(&mut self, serial: NotifId) {
        ...
        let id = self.notifs.get(&serial)
            .expect("invalid NotifId"); // TODO + PANICS: delay closing until id was received
    ...
    fn method_call(&mut self, payload: MethodCall) -> OutgoingId {
        ...
        self.serial += 1;
        let req = Outgoing::MethodCall(serial + 1, payload);
        self.sender.try_send(req).unwrap();
        serial
    }

    fn method_call_with_serial(&mut self, payload: MethodCall, serial: Serial) -> OutgoingId {
        dbg!(serial.inner);
        let req = Outgoing::MethodCall(serial.inner, payload);
        self.sender.try_send(req).unwrap();
        serial.inner
    }

    fn method_reply(&mut self, result: MethodResult<MethodReply>) {
        let req = match result {
            Ok(payload)  => Outgoing::MethodReply(payload),
            Err(payload) => Outgoing::MethodError(payload),
        };
        self.sender.try_send(req).unwrap();
    }

    fn add_match(&mut self, matching: SignalMatch) {
        let call = MethodCall::add_match(matching.rule.clone());
        self.method_call_with_serial(call, Serial::NOREPLY);
    }

    fn remove_match(&mut self, matching: SignalMatch) {
        let call = MethodCall::remove_match(matching.rule.clone());
        self.method_call(call);
    }

    fn request_name<S1>(&mut self, name: S1) -> OutgoingId
      where S1: Into<String> {
        let call = MethodCall::request_name(name.into(), 0x4); // 0x4: don't enqueue if already owned
        self.method_call(call)
    ...
    pub(self) async fn incoming(&mut self) -> io::Result<Incoming> {
        ...
        loop {
            ...
            let incoming = async {
                if let Some(val) = self.incoming.pop() {
                    io::Result::Ok(Either::Incoming(val))
                } else {
            ...
            let outgoing = async {
                let val = self.receiver.recv().await.unwrap();
                Ok(Either::Outgoing(val))
            };
            ...
            match readable.or(outgoing).or(incoming).await? {
                Either::Incoming(incoming) => return Ok(incoming),
                Either::Outgoing(outgoing) => process_request(self, outgoing).await?,
                Either::MoreData => loop {
                    match process_incoming(self) {
                        Ok(result) => self.incoming.push(result),
                        Err(ParseError::Partial) => break,
...
impl Serial {
    pub const NOREPLY: Self = Self { inner: u32::MAX };
    #[track_caller]
    pub fn new(inner: u32) -> Self {
        assert_ne!(inner, Self::NOREPLY.inner);
        Self { inner }
    }
}
...
impl SignalTrigger {
    ...
    pub fn get<T: IsArg>(&mut self, idx: usize) -> Result<T, ArgError> {
        let arg = self.args.get_mut(idx).take()
            .ok_or(ArgError::DoesntExist)?
...
pub enum Outgoing {
    InitialAuth,
    MethodCall(OutgoingId, MethodCall),
    MethodCallVoid(MethodCall),
    MethodReply(MethodReply),
    MethodError(MethodError),
    CloseNotif(OutgoingId),
}
...
impl GenericMessage {
    ...
    pub fn serialize(mut self, serial: Serial) -> Vec<u8> {
        ...
        // unique serial, self.serial is ignored here
        Arg::serialize(serial.inner.pack(), &mut buf);
...
impl MethodError {
    ...
    pub fn new<S1, S2>(to: &MethodCall, name: S1, desc: S2) -> Self
        where S1: Into<Cow<'static, str>>,
              S2: Into<Cow<'static, str>> {
        Self {
            caller: to.caller.clone(),
    ...
    pub fn other<S1>(to: &MethodCall, desc: S1) -> Self
        where S1: Into<Cow<'static, str>> {
        Self {
            caller: to.caller.clone(),
            name: "lsg.other".into(),
    ...
    pub fn unimplemented(to: &MethodCall) -> Self {
        Self {
            ...
            desc: "this member is not implemented, as of now".into(),
        }
    }
    ...
    pub fn serialize(self) -> GenericMessage {
        ...
        msg.args.push(Arg::Simple(SimpleArg::String(self.desc)));
    ...
    pub fn deserialize(mut msg: GenericMessage) -> Result<Self, ParseError> {
        Ok(Self {
            ...
            desc: Cow::Owned(String::unpack(msg.args.remove(0)).ok_or(ParseError::Invalid)?),
        })
    }
...
pub struct Notif {
    pub id: NotifId,
    /// populated when sending using `send_notif`
    outgoing: async_channel::Sender<Outgoing>,
}

impl Notif {

    pub fn build<'d>(summary: &'d str) -> NotifBuilder<'d> {
        NotifBuilder::new(summary)
    }
...
impl Drop for Notif {
    ...
    fn drop(&mut self) {
        ...
        // let id = self.notifs.get(&serial)
        //     .expect("don't drop a notif immediatly, since this will result in closing it");
