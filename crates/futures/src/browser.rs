...
use crate::ffi::{types, waker::{self, waker_clone, waker_drop}};
...
#[unsafe(no_mangle)]
pub extern "C" fn waker_new_boxed_browser(state: *const void) -> *const types::InternalWaker {
    let waker = unsafe { waker::waker_build(types::ExternWaker { state, vtable: &VTABLE }) };
    Box::into_raw(Box::new(waker))
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn waker_clone_boxed(waker: *const types::InternalWaker) -> *const types::InternalWaker {
    let cloned = unsafe { waker_clone(waker) };
    Box::into_raw(Box::new(cloned)).cast_const()
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn waker_drop_boxed(owned: *const types::InternalWaker) {
    unsafe { waker_drop(ptr::read(owned)); } // `InternalWaker` does not get dropped automatically.
