...
impl WindowBackend {
    ...
    pub fn minsize(&self, size: Option<PhysicalSize>) {
        ...
        state.xdg_toplevel.set_min_size(size.x as i32, size.y as i32);
        state.wl_surface.commit();
    ...
    pub fn maxsize(&self, size: Option<PhysicalSize>) {
        ...
        state.xdg_toplevel.set_max_size(size.x as i32, size.y as i32);
        state.wl_surface.commit();
...
impl CustomIconBackend {
    ...
    pub fn new(evl: &EventLoop, size: PhysicalSize, format: IconFormat, data: &[u8]) -> Self {
        ...
        debug_assert!(
            len.get() == size.x as usize * size.y as usize * bytes_per_pixel as usize,
        ...
        let wl_buffer = wl_shm_pool.create_buffer(
            0, size.x as i32, size.y as i32,
            size.x as i32 * bytes_per_pixel, wl_format,
            &evb.qh, ()
        );
...
//         xdg_positioner.set_size(size.x as i32, size.y as i32);
//         xdg_positioner.set_anchor_rect(0, 0, size.x as i32, size.y as i32);
//         drop(parent_guard);
...
//         // zwlr_surface.set_size(size.x as u32, size.y as u32);
        ...
        match event {
            ...
            WlOutputEvent::Mode { flags, width, height, refresh } => {
                if flags.into_result().is_ok_and(|it| it.contains(WlOutputMode::Current)) {
                    info.size = PhysicalSize::new(width as i16, height as i16);
...
impl wayland_client::Dispatch<XdgSurface, Id> for ConnectionState {
    fn event(
        ...
        if let XdgSurfaceEvent::Configure { serial } = event {
            ...
            //     frac_scale_data.viewport.set_destination(size.x as i32, size.y as i32);
            // };
    ...
    fn event(
        ...
        if let XdgToplevelEvent::Configure { width, height, states } = event {
            ...
            if !zerosized {
                ...
                window.size = PhysicalSize::new(width as i16, height as i16)
                    .scale(window.scale);
    ...
    fn event(
        ...
        match event {
             ...
             WlPointerEvent::Enter { surface, surface_x, surface_y, serial } => {
                ...
                evl.mouse.pos = PhysicalPoint::new(x, window.size.y as i16 - y)
                    .scale(window.scale);
             ...
             WlPointerEvent::Motion { surface_x, surface_y, .. } => {
                 ...
                 evl.mouse.pos.x = x;
                 evl.mouse.pos.y = window.size.y as i16 - y;
